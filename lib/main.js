...
var plugdjPageMod = pageMod.PageMod({
	...
	onAttach: function(worker) {
		...
		if (preferences.lastfmSession)
			worker.port.emit('setSession', preferences.lastfmSession);

		worker.port.emit('setScrobbling', preferences.lastfmScrobbling);
		worker.port.emit('setUpdating', preferences.lastfmUpdating);
...
function setSession(session) {
	...
	if (plugdjPort !== false)
		plugdjPort.emit('setSession', session);
}
...
function setScrobbling(scrobble) {
	...
	if (plugdjPort !== false)
		plugdjPort.emit('setScrobbling', scrobble);
}
...
function setUpdating(update) {
	...
	if (plugdjPort !== false)
		plugdjPort.emit('setUpdating', update);
}
